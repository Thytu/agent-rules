...
// (see docs/rules/contract-evolution.md).
...
export const noCompatShims = {
	meta: {
		...
		messages: {
			compatComment:
				'Backward-compatibility marker in a comment ("{{match}}"). Nothing outside this repo calls our internals: update every caller in this change, migrate data forward, and DELETE the old path — git is the archive. Owner-decided external boundaries are the only exception. See docs/rules/contract-evolution.md.',
			compatName:
				'Identifier "{{name}}" declares a legacy/deprecated thing. Don\'t keep two ways to do the same thing — replace the old one and update its callers in this change. See docs/rules/contract-evolution.md.',
		},
	},
	create(context) {
