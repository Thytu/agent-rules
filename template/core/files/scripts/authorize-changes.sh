...
while IFS= read -r -d '' path; do
	if [ "$mode" = source ]; then
		case "$path" in AGENTS.md|CLAUDE.md|package.json|pnpm-lock.yaml|.npmrc|biome.json|eslint.config.mjs|init.sh|template/*|.github/*|.githooks/*|scripts/setup-source.sh|scripts/setup-github.sh|docs/rules/*|docs/profiles/*)
