...
while IFS= read -r -d '' path; do
	[ "$mode" = source ] && case "$path" in init.sh|template/*|.agents/*|.claude/*|package.json|pnpm-lock.yaml|.github/*|scripts/setup-source.sh|scripts/setup-github.sh|docs/rules/*|docs/profiles/*) matches=1 ;; esac
