...
if grep -R -n 'REPLACE_ME' "$root/SCOPE.md" "$root/VERIFICATION.md" "$root/docs/rules" "$root/docs/profiles"; then fail "active product contracts still contain REPLACE_ME"; fi
