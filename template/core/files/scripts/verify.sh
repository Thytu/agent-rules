...
if [ "$mode" = source ]; then
	for path in AGENTS.md CLAUDE.md README.md init.sh package.json pnpm-lock.yaml .agents/skills docs/rules template/core/files/scripts/verify.sh template/rust/files template/rust/agent-map.rows template/typescript/files template/typescript/agent-map.rows test/generator tooling/pr-review .github/workflows/ci.yml .github/workflows/pull-request.yml .github/workflows/ai-review.yml; do require "$path"; done
